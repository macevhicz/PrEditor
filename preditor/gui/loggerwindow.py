...
class LoggerWindow(Window):
    ...
    def createActions(self):
        ...
        action.setShortcut(Qt.Modifier.CTRL | Qt.Key.Key_M)
    ...
    def startFileSystemMonitor(self):
        ...
        self.openFileMonitor.addPath(self.prefsPath())
    ...
    def change_to_workbox_version_text(self, versionType):
        ...
        isFirstWorkbox = idx is not None and idx == 0
        isLastWorkbox = idx is not None and idx + 1 == count
        ...
        # save any thing they've typed, then if going previous, instead to back
        # 2 version, since we just created a new version
        if isLastWorkbox and isDirty:
            workbox_widget.__save_prefs__()
            if forPrevious:
                versionType = prefs.VersionTypes.TwoBeforeLast
    ...
    def setFileWatchingEnabled(self, filename, state):
        ...
        if filename == prefs_path:
            ...
            for info in self.uiWorkboxTAB.all_widgets():
                ...
                if Path(editor.filename()).as_posix() == Path(filename).as_posix():
                    self.updateLink(editor)
        self.updateTabColorsAndToolTips()

    def updateLink(self, editor):
        """If an editor is linked to a file, update it here. File monitoring
        will automatically be deleted by QFileSystemWatcher if the file no
        longer exists.

        Args:
            editor (WorkboxMixin): The editor which has potentially been updated.
        """
        filename = editor.filename()
        if Path(filename).is_file():
            editor.__load__(filename)
        ...
        pref.update(
            {
                ...
                'selectText': self.uiSelectTextCHK.isChecked(),
    ...
    def save_prefs(self, pref, at_prefs_update=False):
        ...
        self.setFileWatchingEnabled(self.prefsPath(), False)
        shutil.copy(temp_path, path)
        self.setFileWatchingEnabled(self.prefsPath(), True)
        temp_path.unlink()
    ...
    def restorePrefs(self, skip_geom=False):
        ...
        self.uiSelectTextCHK.setChecked(pref.get('selectText', True))
        ...
        self.uiMaxNumBackupsSPIN.setValue(pref.get('max_num_backups', 50))
    ...
    def addRecentlyClosedWorkbox(self, workbox):
        ...
        max_recent_workboxes = self.uiMaxNumRecentWorkboxesSPIN.value()
        workboxesData = workboxesData[:max_recent_workboxes]

        for workboxesDatum in workboxesData:
            workbox_name = workboxesDatum.get("workbox_name")
            text_sample = workboxesDatum.get("text_sample")
            action = self.uiClosedWorkboxesMENU.addAction(workbox_name)
            action.setData(workboxesDatum)
            action.triggered.connect(self.recentWorkboxActionTriggered)
            action.setToolTip(text_sample)
    ...
    def buildClosedWorkBoxMenu(self, closedWorkboxData=None):
        ...
        for datum in closedWorkboxData:
            if datum:
                name = datum.get("workbox_name")
                action = self.uiClosedWorkboxesMENU.addAction(name)
                action.setData(datum)
                action.setToolTip(datum.get("text_sample", ""))
                action.triggered.connect(self.recentWorkboxActionTriggered)
